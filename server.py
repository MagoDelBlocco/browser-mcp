    ...
    for attempt in range(1, SEARCH_RETRIES + 1):
        try:
            ...
            if attempt == SEARCH_RETRIES:
                break
            print(f"[search] {type(e).__name__} — retry {attempt}/{SEARCH_RETRIES} in {delay:.1f}s", flush=True)
...
async def _sample_text(ctx, prompt: str, system: str, max_tokens: int = 200) -> str:
    ...
    try:
        ...
        print(f"[sampling] unavailable ({type(e).__name__}), using fallback", flush=True)
        return ""
...
async def expand_query(ctx, query: str, limit: int = 2) -> list[str]:
    ...
    if variants:
        print(f"[sampling] query variants: {variants}", flush=True)
    return variants
...
async def triage_results(ctx, query: str, hits: list[dict[str, Any]], take: int) -> list[dict[str, Any]]:
    ...
    for token in re.findall(r"\d+", text):
        ...
        if len(picked) >= take:
            break
    if picked:
        print(f"[sampling] triaged to {[h['id'] for h in picked]}", flush=True)
        return picked
    return hits[:take]
...
async def assess_coverage(ctx, question: str, docs: list[dict[str, Any]]) -> str:
    ...
    if first_line.lower() == question.lower():
        return ""
    print(f"[research] follow-up query: {first_line!r}", flush=True)
    return first_line
...
async def research(
    ...
    for hop in range(1, depth + 1):
        ...
        if not follow_up:
            if depth > 1:
                print(f"[research] stopping after hop {hop} — coverage judged sufficient", flush=True)
            break
        hop_query = follow_up
...
def main():
    if BIND_HOST not in {"127.0.0.1", "localhost", "::1"} and not AUTH_TOKEN:
        print(
            f"[warn] listening on {BIND_HOST} with no AUTH_TOKEN — anyone who can reach "
            f"port {PORT} can drive this browser. Publish it to localhost only.",
            flush=True,
        )
